...
REDIRECT_URI = 'https://nira.f5.si/cgi/discord_auth/ss_server.py'
...
def main():
    ...
    try:
        ...
        return
    with open(f"{nira_bot_dir}/steam_server_list.nira","rb") as f:
        ss_list = pickle.load(f)
    ss_list_keys = list(ss_list.keys())
    key_i = 0
    url = ""
    for i in range(len(guild)):
        if int(guild[i]) not in ss_list_keys:
            continue
        for i in range(int((len(ss_list[ss_list_keys[key_i]])-1)/2)):
            # {ss_list[ss_list_keys[key_i]][f'{i+1}_ad'][0]}
            # [server_name]=[address]:[port]
            url = url + f"{ss_list[ss_list_keys[key_i]][f'{i+1}_nm']}={ss_list[ss_list_keys[key_i]][f'{i+1}_ad'][0]}:{ss_list[ss_list_keys[key_i]][f'{i+1}_ad'][1]}?"
        key_i = key_i + 1
    url = url[:-1]
    print(f"""
    <script>
        sessionStorage.setItem("guilds", "{guild}")
        data = localStorage.getItem("ss_list")
    """ + """
        if (data == null) {
    """ + f"""
        data = "{url}"
    ...
    """ + f"""
        ...
        console.log(data);
        localStorage.setItem("ss_list", data);
